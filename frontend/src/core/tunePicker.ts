...
function targetWeight(score: number, target: number): number {
...
export function tuneWeight(tune: Tune, filters: Filters, now?: number): number {
  return (
    targetWeight(tune.obscurity_score, filters.obscurity) *
    targetWeight(tune.difficulty_score, filters.difficulty) *
...
export function pickRandomTune(
  ...
  const now = Date.now();
  const weights = choose.map((t) => tuneWeight(t, filters, now));
